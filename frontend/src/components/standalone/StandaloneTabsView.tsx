...
import { AlertTriangle, Code2, Copy, FileText, FolderOpen, Loader2, MessageSquarePlus, Terminal as TerminalIcon, X, type LucideIcon } from 'lucide-react';
...
import { hasWorkspaceFileDragPayload, readWorkspaceFileDragPayload, setWorkspaceFileDragData } from '../../lib/workspaceDrag';
...
function StandaloneTabStrip({ tabsState }: { tabsState: DesktopStandaloneTabsState }) {
  ...
  const dragTab = (event: DragEvent<HTMLDivElement>, tab: DesktopStandaloneTab) => {
    ...
    event.dataTransfer.setData('text/plain', tab.filePath ?? tab.title);
    if (tab.filePath) {
  ...
  const allowTabDrop = (event: DragEvent<HTMLDivElement>) => {
    const hasStandaloneTab = event.dataTransfer.types.includes(STANDALONE_TAB_MIME);
    if (!hasStandaloneTab && !hasWorkspaceFileDragPayload(event.dataTransfer)) return;
    event.preventDefault();
    event.dataTransfer.dropEffect = hasStandaloneTab ? 'move' : 'copy';
  };

  const moveDroppedTab = async (event: DragEvent<HTMLDivElement>) => {
    const raw = event.dataTransfer.getData(STANDALONE_TAB_MIME);
    if (raw && tabsState.groupId) {
      event.preventDefault();
      try {
  ...
  const contextMenuTop = contextMenu
    ? typeof window === 'undefined'
      ? contextMenu.y
      : Math.min(contextMenu.y, Math.max(8, window.innerHeight - 220))
    : 0;
  ...
  const copyTabPath = async (tab: DesktopStandaloneTab) => {
    if (!tab.filePath) return;
    ...
    <>
      ...
      {contextMenu && contextTab && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          <StandaloneTabMenuButton
            icon={Copy}
            label={t('rightPanel.fileMenu.copyPath')}
            disabled={!contextTab.filePath}
            onClick={() => void copyTabPath(contextTab)}
          />
