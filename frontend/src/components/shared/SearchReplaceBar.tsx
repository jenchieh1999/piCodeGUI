...
export function SearchReplaceBar({
  ...
  }, [activeIndex, matches, onSelectMatch, textInputRef, visible]);
  ...
  const replaceCurrent = () => {
    ...
    const match = matches[activeIndex];
    if (!match) return;
    const nextText = replaceTextMatch(text, match, query, replacement, options);
    onTextChange(nextText);
    window.requestAnimationFrame(() => onSelectMatch?.(match.start, match.start + replacement.length));
  };
