...
export function isMarkdownStandaloneRoute(): boolean {
  if (typeof window === 'undefined') return false;
  return new URLSearchParams(window.location.search).get('desktopView') === 'markdown';
}

export function MarkdownStandaloneView() {
  ...
  const windowTitle = useMemo(
    () => (route.filePath ? `${getDisplayName(route.filePath)} - ${t('standalone.markdownTitleSuffix')}` : t('standalone.markdownTitle')),
    [route.filePath, t]
  );
    ...
    async function boot() {
      try {
        if (!route.sessionId || !route.filePath) {
          throw new Error(t('standalone.markdownRouteMissing'));
        }
  ...
  return (
    <StandaloneFrame title={windowTitle}>
      <div className="min-h-0 flex-1 overflow-hidden">
        <MarkdownFileReader sessionId={route.sessionId} filePath={route.filePath} />
      </div>
    </StandaloneFrame>
  );
}
