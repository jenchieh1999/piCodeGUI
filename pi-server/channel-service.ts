import { createDecipheriv, createHash, createHmac, randomBytes, randomInt, randomUUID, timingSafeEqual } from 'node:crypto';
...
interface ChannelSessionResolution {
  session: SessionData;
...
export class ChannelService {
  ...
  async handleRequest(req: IncomingMessage): Promise<ChannelHttpResponse | null> {
    ...
    try {
      ...
      if (parts.length === 4 && parts[3] === 'inbound' && req.method === 'POST') {
        ...
        const authError = this.authorizeGenericInbound(req, url, channel, body);
  ...
  private authorizeGenericInbound(
    req: IncomingMessage,
    url: URL,
    channel: ChannelConfigData,
    body: Record<string, unknown>,
  ): ChannelHttpResponse | null {
    if (!channel.verificationToken) {
      this.patchChannelRuntimeState(channel.id, {
        lastError: 'Generic inbound rejected because no verification token is configured.',
      });
      return this.json(403, { error: 'Generic inbound requires a channel verification token.' });
    }

    const token = extractChannelAuthToken(req, url, body);
    if (!safeEqualString(channel.verificationToken, token)) {
      this.patchChannelRuntimeState(channel.id, { lastError: 'Generic inbound token mismatch.' });
      return this.json(401, { error: 'Invalid channel token' });
    }
  ...
  private async handleFeishuEvent(channel: ChannelConfigData, body: Record<string, unknown>): Promise<ChannelHttpResponse> {
    const wasEncrypted = Boolean(normalizeString(body.encrypt));
    ...
    if (channel.verificationToken && !safeEqualString(channel.verificationToken, token)) {
    ...
    if (!channel.verificationToken && !wasEncrypted) {
      this.patchChannelRuntimeState(channel.id, {
        lastError: 'Feishu event rejected because neither verification token nor encryption key proof was available.',
      });
      return this.json(403, { error: 'Feishu event requires verification token or encrypted delivery.' });
    }
  ...
  private handleWechatVerify(channel: ChannelConfigData, url: URL): ChannelHttpResponse {
    ...
    if (!channel.verificationToken) {
      this.patchChannelRuntimeState(channel.id, {
        lastError: 'WeChat verification rejected because no verification token is configured.',
      });
      return { status: 403, text: 'verification token is required', headers: plainTextHeaders() };
    }
  ...
  private async handleWechatEvent(channel: ChannelConfigData, url: URL, rawBody: string): Promise<ChannelHttpResponse> {
    if (!channel.verificationToken) {
      this.patchChannelRuntimeState(channel.id, {
        lastError: 'WeChat event rejected because no verification token is configured.',
      });
      return { status: 403, text: 'verification token is required', headers: plainTextHeaders() };
    }
  ...
  private async acceptInbound(
    ...
    const resolved = this.resolveChannelSession(channel, inbound, agent);
    if (!resolved) {
    ...
    const routedChannel = resolved.channel;
    if (resolved.created) {
      this.options.broadcast({ type: 'session_created', session, messages: [] });
    }

    const updatedChannel = this.patchChannelRuntimeState(routedChannel.id, {
      lastEventAt: Date.now(),
      lastRecipientId: recipientForInbound(routedChannel, inbound),
      lastContextToken: inbound.contextToken,
      lastError: undefined,
    });

    const displayText = formatInboundDisplay(routedChannel, inbound, agent);
    ...
    const promptText = formatAgentPrompt(agent, formatChannelPrompt(routedChannel, inbound));
    const replyChannel = {
      ...routedChannel,
      ...(updatedChannel ?? {}),
      lastRecipientId: recipientForInbound(routedChannel, inbound),
  ...
  private resolveChannelSession(
    ...
    inbound: ChannelInboundEventData,
    agent: AgentConfigData | null,
  ): ChannelSessionResolution | null {
    const agentProjectPath = agent?.projectPath?.trim();
    const conversationKey = channelConversationKey(channel, inbound);
    const bindings = normalizeSessionBindings(channel.sessionBindings);
    const boundSessionId = bindings[conversationKey];
    if (boundSessionId) {
      const session = getSession(boundSessionId);
      if (session) {
        maybeApplySessionAgentModel(session.id, agent);
        return { session, channel, created: false };
      }
      delete bindings[conversationKey];
      this.patchChannelRuntimeState(channel.id, { sessionBindings: bindings });
    }

    if (!channel.autoCreateSession) {
      if (!agentProjectPath && channel.defaultSessionId) {
    ...
    const projectPath = agentProjectPath || channel.defaultProjectPath?.trim() || process.cwd();
    const session = createSession(projectPath, { projectName: agent?.name || channel.name || basename(projectPath) });
    maybeApplySessionAgentModel(session.id, agent);

    const patch: Partial<Pick<ChannelConfigData, 'defaultSessionId' | 'sessionBindings'>> = {
      sessionBindings: {
        ...bindings,
        [conversationKey]: session.id,
      },
    };
    if (!channel.defaultSessionId) {
      patch.defaultSessionId = session.id;
    }

    const updated = this.patchChannelRuntimeState(channel.id, patch);
    return {
      session,
      channel: updated ?? { ...channel, ...patch },
      created: true,
    };
  }
...
function normalizeStoredChannel(raw: unknown): ChannelConfigData | null {
  ...
  return {
    ...
    sessionBindings: normalizeSessionBindings(record.sessionBindings),
...
function channelConversationKey(channel: ChannelConfigData, inbound: ChannelInboundEventData): string {
  if (channel.provider === 'feishu') {
    if (inbound.chatId) return `feishu:chat:${inbound.chatId}`;
    if (inbound.userId) return `feishu:user:${inbound.userId}`;
  }

  if (channel.provider === 'wechat') {
    if (inbound.chatId) return `wechat:chat:${inbound.chatId}`;
    if (inbound.userId) return `wechat:user:${inbound.userId}`;
  }

  const recipient = recipientForInbound(channel, inbound) || defaultRecipientForInbound(channel, inbound) || 'default';
  return `${channel.provider}:recipient:${recipient}`;
}
...
function normalizeSessionBindings(value: unknown): Record<string, string> {
  const record = normalizeRecord(value);
  const bindings: Record<string, string> = {};
  for (const [rawKey, rawSessionId] of Object.entries(record)) {
    const key = normalizeString(rawKey);
    const sessionId = normalizeString(rawSessionId);
    if (key && sessionId) bindings[key] = sessionId;
  }
  return bindings;
}
...
function extractChannelAuthToken(req: IncomingMessage, url: URL, body: Record<string, unknown>): string | undefined {
  const queryToken = url.searchParams.get('token') ?? undefined;
  return extractBearer(firstHeader(req.headers.authorization))
    || firstHeader(req.headers['x-pi-channel-token'])
    || queryToken
...
function safeEqualString(expected: string | undefined, actual: string | undefined | null): boolean {
...
async function readRawBody(req: IncomingMessage): Promise<string> {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_CHANNEL_BODY_BYTES) {
...
function corsHeaders(): Record<string, string> {
  return {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Pi-Channel-Token',
  };
}
