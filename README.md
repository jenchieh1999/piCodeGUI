...
> Current status: private pre-release, version `0.1.0`. The main desktop workflow is usable, but formal release hardening still depends on signed installers, a configured update feed, more regression coverage, and real upgrade-path testing.
...
- Theme system with 20+ built-in styles, user-created themes, deletable custom themes, font settings, chat background image support, and additional Claude Code, Codex, Trae, Cyberpunk, and Star Wars-inspired themes.
- Windows build pipeline through `electron-builder`, currently configured for NSIS installers on `x64` and `ia32`.
